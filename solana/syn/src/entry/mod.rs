...
/// The struct containing the parsed contents required to convert the user's annotated module into the proper program configurations.
/// * `leftover_content`: Any declarations in the module that aren't named functions.
/// * `instruction_enum`: The built-out program instruction enum derived from the functions and their arguments.
/// * `declared_functions`: The user's declared functions as-is.
/// * `processor`: The program's processor, built into a function `process_instruction`.
#[derive(Debug)]
...
impl From<ItemMod> for NautilusEntrypoint {
    /// Converts the user's annotated module into the `NautilusEntrypoint` struct.
    ///
    /// All of the work to build out the entrypoint, processor, and IDL for a Nautilus program is done here.
    /// During this conversion, the module (`ItemMod`) is broken down into components, and the functions declared by the user
    /// are extracted and used to build out various child structs such as `NautilusEntrypointEnum` and `NautilusEntrypointEnumVariant`.
    ///
    /// Using information extracted from the user's manifest (`Cargo.toml`) and the entirety of their crate, this function builds the `NautilusEntrypointEnum`,
    /// which basically dissolves to the required components.
    ///
    /// For more specific information see the documentation for `NautilusEntrypointEnum` and `NautilusEntrypointEnumVariant`.
...
impl Parse for NautilusEntrypoint {
    /// Parses the user's defined module into a `syn::ItemMod`, which is an already pre-fabricated function,
    /// and calls Into<NautilusEntrypoint> to fire the `from(value: ItemMod)` in the trait implementation `impl From<ItemMod> for NautilusEntrypoint`,
...
impl ToTokens for NautilusEntrypoint {
    /// Extends the existing compiler tokens by the tokens generated by the `NautilusEntrypoint`.
...
impl From<&NautilusEntrypoint> for TokenStream {
    /// Converts each component of the `NautilusEntrypoint` into the proper tokens for the compiler.
