...
///
/// Note: `seeds(..)` and `pda(..)` are derived from the derive macro - since
/// their arguments will vary.
pub trait NautilusAccountData: BorshDeserialize + BorshSerialize + Clone + Default {
    const DISCRIMINATOR_STR: &'static str;

    /// The 8-bit discriminator applied to this account as a prefix on any
    /// account data containing this data type.
...
/// The trait that represents account data for record-based, SQL-friendly
/// Nautilus accounts.
///
/// This type of account is a program-derived address account (PDA) but can be
/// treated like a record in a database table, with the data scheme (struct)
/// being the schema of the table.
pub trait NautilusRecordData: BorshDeserialize + BorshSerialize + Clone + Default {
    ...
    /// The primary key of this particular record's account data type. This will
    /// return the value of whichever field has been declared as the primary
    /// key for this table.
    fn primary_key(&self) -> Vec<u8>;

    /// The seeds used to derive the program-derived address of this account.
    ///
    /// Accessible through the account's data type since some parameters for
    /// seeds may be based on fields in the data.
    fn seeds(&self) -> Vec<Vec<u8>> {
        vec![Self::TABLE_NAME.as_bytes().to_vec(), self.primary_key()]
    }

    /// Returns the program-derived address and bump for an account containing
    /// this data.
    ///
    /// Accessible through the account's data type since some parameters for
    /// seeds may be based on fields in the data.
...
///
/// When you define a struct with the `#[derive(nautilus::State)]` macro, any
/// accounts created with this data scheme will have seeds defined and inherit
/// PDA functionality.
///
/// Note: `seeds(..)` and `pda(..)` are derived from the derive macro - since
/// their arguments will vary.
...
///
/// When you define a struct with the `#[derive(nautilus::Table)]` macro, any
/// accounts created with this data scheme can be considered records, and the
/// struct itself can be considered a schema for a "table".
///
/// This trait is where things like autoincrement and authority checks come into
/// play.
