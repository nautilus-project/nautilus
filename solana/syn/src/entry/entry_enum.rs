...
/// The struct used to house all of the "variants" of type `NautilusEntrypointEnumVariant` which dissolve to
...
impl NautilusEntrypointEnum {
    ...
    ///
    /// This action will simply convert the user's declared functions into `NautilusEntrypointEnumVariant` instances, which dissolve to
    /// the required components for building out the generated program.
...
impl From<&NautilusEntrypointEnum> for (TokenStream, TokenStream, Vec<IdlInstruction>) {
    /// Maps each `NautilusEntrypointEnumVariant` into the proper components and dissolves itself into
    /// the required components for building out the generated program.
    ///
    /// Consider the `fold` operation on the `variants` field, which returns (`variants`, `match_arms`, `idl_instructions`):
    /// * `variants`: The variants of the instruction enum for the program.
    /// * `match_arms`: The match arms of the processor which will process whichever instruction enum variant (and its arguments) is provided to the program.
    /// * `idl_instructions`: The IDL instructions derived from the declared functions - to be used in generating the IDL.
    ///
    /// Consider the return type of the function itself - defined at the trait level: (`TokenStream`, `TokenStream`, `Vec<IdlInstruction>`):
